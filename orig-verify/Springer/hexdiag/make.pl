...
while ($filename = shift @files) {
	...
	open INFILE, "$filename";
        while ($line = <INFILE>) {
	    if ($line =~ /BoundingBox\D*(\d*)\D*(\d*)\D*(\d*)\D*(\d*)/i) {
...
sub DetectBoundingPiece {
    ...
    if ($line =~ /DrawHexBoard/ || $line =~ /HexShannonGrid/) {
	...
	&AddBoundingPiece(0,0);
	&AddBoundingPiece(0,$dimX-1);
	&AddBoundingPiece($dimX-1,0);
    }
    elsif ($line =~ /HexBoardCoordinates/) {
	&AddBoundingPiece(0,1);
	&AddBoundingPiece(1,0);
	&AddBoundingPiece(0,$dimY);
	&AddBoundingPiece(1,$dimY+1);
	&AddBoundingPiece($dimX,0);
	&AddBoundingPiece($dimX+1,1);
	&AddBoundingPiece($dimX,$dimY+1);
	&AddBoundingPiece($dimX+1,$dimY);
    }
